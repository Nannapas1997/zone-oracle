...
│   ├── subscriptionStore.ts   ← in-memory subscription store
│   └── useSubscription.ts     ← ดึง subscription status จาก NextAuth session
├── pages/
│   ├── index.tsx              ← Landing page
│   ├── pricing.tsx            ← Pricing + Stripe checkout
│   ├── _app.tsx               ← NextAuth SessionProvider
│   ├── sign-in/index.tsx      ← หน้า login (email + Google)
...
│       │   └── webhook.ts         ← รับ events จาก Stripe → update store
│       └── subscription/
...
3. Authorized redirect URIs → เพิ่ม `http://localhost:3000/api/auth/callback/google`
...
NEXTAUTH_SECRET=your_random_secret        # รัน: openssl rand -base64 32
...
3. อัพเดท `NEXTAUTH_URL` เป็น domain จริง เช่น `https://zone-oracle.com`
4. อัพเดท Google OAuth → Authorized redirect URIs เพิ่ม `https://zone-oracle.com/api/auth/callback/google`
...
ผู้ใช้เข้า zone-oracle.com
    ...
    Landing page (index.tsx)
         ↓ Sign In (NextAuth — Email หรือ Google)
    สร้าง session
         ↓
    ...
    Stripe Checkout (create-checkout.ts)
         ↓ จ่ายเงิน
    Stripe Webhook → อัพเดท subscriptionStore (isActive: true)
         ↓
